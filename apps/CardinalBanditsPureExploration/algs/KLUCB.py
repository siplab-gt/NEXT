        ...
        if S != None:
            ...
            for i in range(n):
                if T[i] == 0:
                    ...
                    UCB[i] = computeUCB(
                        muhat=(mu[i]-1)/2, threshold=(numpy.log(2*T[i]*T[i]/delta)/T[i]))

            # sort by -UCB first then break ties randomly
            priority_list = numpy.lexsort(
                (numpy.random.randn(n), -UCB)).tolist()

            butler.algorithms.set_many(key_value_dict={
                                       'priority_list': priority_list, 'priority_list_cnt': 0, 'Xsum': Xsum, 'X2sum': X2sum, 'T': T})
