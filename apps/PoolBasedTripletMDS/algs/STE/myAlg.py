...
class MyAlg:
    ...
    def getQuery(self, butler):
        ...
        while time.time()-t_start < t_max:
            ...
            p = 0
            for i in range(n):
                p += utilsSTE.getSTETripletProbability(
                    X[b], X[c], X[i]) * tau[a, i]

            taub = list(tau[a])
            for i in range(n):
                taub[i] = taub[i] * \
                    utilsSTE.getSTETripletProbability(X[b], X[c], X[i])
            taub = taub/sum(taub)

            tauc = list(tau[a])
            for i in range(n):
                tauc[i] = tauc[i] * \
                    utilsSTE.getSTETripletProbability(X[c], X[b], X[i])
            tauc = tauc/sum(tauc)

            entropy = -p * \
                utilsSTE.getEntropy(taub)-(1-p)*utilsSTE.getEntropy(tauc)
    ...
    def full_embedding_update(self, butler, args):
        ...
        emp_loss_old, hinge_loss_old, log_loss_old = utilsSTE.getLoss(
            X_old, S, alpha)
        X, tmp = utilsSTE.computeEmbeddingWithEpochSGD(
            n, d, S, alpha, max_num_passes=16, epsilon=0, verbose=verbose)
        t_start = time.time()
        X, emp_loss_new, hinge_loss_new, log_loss_new, acc = utilsSTE.computeEmbeddingWithGD(
            X, S, alpha, max_iters=1, epsilon=epsilon, verbose=verbose)
        k = 1
