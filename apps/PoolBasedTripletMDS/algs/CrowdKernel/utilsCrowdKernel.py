...
def main():
    ...
    X, emp_loss_train = computeEmbedding(
        n, d, Strain, mu=.01, num_random_restarts=2, epsilon=0, verbose=True)
...
def getLoss(X, S):
    ...
    for q in S:
        ...
        log_loss = log_loss - \
            log(getCrowdKernelTripletProbability(
                X[q[0]], X[q[1]], X[q[2]], mu=.01))
...
def getGradient(X, S, mu):
    ...
    for q in S:
        i, j, k = q
        num = max(mu + norm(X[k]-X[j])*norm(X[k]-X[j]),
                  realmin)  # max(mu + D[k,j] , realmin)
        # max(2*mu + D[k,i] + D[k,j], realmin)
        den = max(2*mu + norm(X[k]-X[i])*norm(X[k]-X[i]) +
                  norm(X[k]-X[j])*norm(X[k]-X[j]), realmin)
        G[i] += 2/den * (X[k]-X[i])
...
def computeEmbedding(n, d, S, mu=.01, num_random_restarts=0, max_num_passes=0, max_norm=0, epsilon=0.01, verbose=False):
    ...
    while num_restarts < num_random_restarts:
        ...
        X, acc = computeEmbeddingWithEpochSGD(
            n, d, S, mu, max_num_passes=max_num_passes, epsilon=0., verbose=verbose)
        te_sgd = time.time()-ts

        print("Gradient Descent")
        ts = time.time()
        X_new, emp_loss_new, log_loss_new, hinge_loss_new, acc_new = computeEmbeddingWithGD(
            X, S, mu, max_iters=50, max_norm=max_norm, epsilon=epsilon, verbose=verbose)
...
def computeEmbeddingWithGD(X, S, mu, max_iters=0, max_norm=0, epsilon=0.01, c1=0.0001, rho=.7, verbose=False):
    ...
    while t < max_iters:
        ...
        # check losses
        if verbose:
            print("iter=%d,   emp_loss=%f,   hinge_loss=%f,   log_loss=%f,   rel_avg_grad=%f,   rel_max_grad=%f,   a=%f,   i_t=%d" % (
                t, emp_loss_k, hinge_loss_k, log_loss_k, rel_avg_grad, rel_max_grad, alpha, inner_t))
