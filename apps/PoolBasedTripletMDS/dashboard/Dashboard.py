...
class MyAppDashboard(AppDashboard):
    ...
    def test_error_multiline_plot(self, app, butler):
        ...
        test_S = butler.queries.get(
            pattern={'exp_uid': app.exp_uid, 'alg_label': test_alg_label})
        x_min = float('inf')
        x_max = -float('inf')
        y_min = float('inf')
        y_max = -float('inf')
                    ...
                    for query in test_S:
                        if 'q' in query:
                            ...
                            score = numpy.dot(Xd[j], Xd[j]) - 2*numpy.dot(
                                Xd[j], Xd[k]) + 2*numpy.dot(Xd[i], Xd[k]) - numpy.dot(Xd[i], Xd[i])
                            if score > 0:
                                number_correct += 1.0
    ...
    def most_current_embedding(self, app, butler, alg_label):
        ...
        for idx, target in enumerate(embedding):
            ...
            try:
                # takes first two components, (could be replaced by PCA)
