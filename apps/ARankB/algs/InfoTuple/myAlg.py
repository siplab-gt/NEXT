...
class MyAlg:
    ...
    def getQuery(self, butler, participant_uid):
        ...
        if not butler.participants.exists(uid = participant_uid, key='embedding'):
            ...
            butler.participants.set(uid=participant_uid, key='embedding', value=X_part.tolist())
    ...
    def incremental_embedding_update(self, butler, args):
        ...
        embedder = SOE(n_components=d, random_state=42, n_init=10, backend='torch', max_iter=20, margin=5) # Embedding algorithm to get the embeddings
    ...
    def full_embedding_update(self, butler, args):
        ...
        embedder = SOE(n_components=d, random_state=seed, n_init=10, backend='torch', max_iter=20, margin=5)
        X = np.array(butler.algorithms.get(key='X'))
        n, d = X.shape
        X = embedder.fit_transform(responses, n_objects=n, init=X)
        #Debug 
