    ...
    while True:
        try:
            ...
            next.utils.debug_print(
                "Failed to connect to database ({}), retrying in {}s".format(e, backoff_dt))
...
class App_Wrapper:
    def __init__(self, app_id, exp_uid, db, ell):
        ...
        self.butler = Butler(
            app_id, exp_uid, self.next_app.myApp.TargetManager, db, ell)

    def getModel(self, args_in_json):
        response, dt = next.utils.timeit(self.next_app.getModel)(
            self.next_app.exp_uid, args_in_json)
...
def apply(app_id, exp_uid, task_name, args_in_json, enqueue_timestamp):
    ...
    time_enqueued = delta_datetime.seconds + delta_datetime.microseconds / \
        1000000.
    ...
    print('#### Finished %s,  time_enqueued=%s,  execution_time=%s ####' %
          (task_name, time_enqueued, dt))
    return return_value
...
def apply_dashboard(app_id, exp_uid, args_in_json, enqueue_timestamp):
    ...
    if DEBUG_ON:
        next.utils.debug_print('#### Finished Dashboard %s, time_enqueued=%s,  execution_time=%s ####' % (
            stat_id, time_enqueued, dt), color='white')
...
def apply_sync_by_namespace(app_id, exp_uid, alg_id, alg_label, task_name, args, namespace, job_uid, enqueue_timestamp, time_limit):
    ...
    try:
        ...
        butler = Butler(app_id, exp_uid, target_manager,
                        db, ell, alg_label, alg_id)
        response, dt = next.utils.timeit(
            getattr(next_alg, task_name))(butler, args)
        log_entry_durations = {
            'exp_uid': exp_uid, 'alg_label': alg_label, 'task': 'daemonProcess', 'duration': dt}
