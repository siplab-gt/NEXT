...
def load_doc(filename, base_path):
    ...
    with open(os.path.join(base_path, filename)) as f:
        ref = yaml.load(f.read(), Loader=yaml.SafeLoader)
        ds = []
...
def check_format(doc):
    ...
    for x in doc:
        if 'args' in doc[x]:
            errs += check_format_helper({'type': 'dict',
                                        'values': doc[x]['args']}, 'args/'+x)
        if 'rets' in doc[x]:
...
def check_format_helper(doc, name):
    ...
    diff = set(doc.keys()) - {'type', 'description',
                              'values', 'optional', 'default'}
    if len(diff) > 0:
        errs += ["{}: extra keys in spec: {}".format(
            name, ", ".join(list(diff)))]
    ...
    if doc['type'] in DICT:
        for x in doc['values']:
            errs += check_format_helper(doc['values']
                                        [x], '{}/{}'.format(name, x))
...
def verify(input_dict, reference_dict):
    ...
    """
    input_dict, messages = verify_helper(
        "", input_dict, {'type': 'dict', 'values': reference_dict})
...
def verify_helper(name, input_element, reference_dict):
    ...
    if reference_dict['type'] in DICT:
        if not isinstance(input_element, (dict)):
            ...
            if len(l1) > 0:
                ans += [{"name": name,
                         "message": "extra keys in input: " + ",".join(l1)}]
            else:
                ok = True
                for k in l2:
                ...
                if (ok):
                    for k in input_element:
                        input_element[k], temp_ans = verify_helper(
                            name + '/' + k, input_element[k], reference_dict['values'][str(k)])
                        ans += temp_ans
        ...
        if not isinstance(input_element, (list)):
            ...
            for i in range(len(input_element)):
                input_element[i], temp_ans = verify_helper(
                    name+'/'+str(i), input_element[i], reference_dict['values'])
                ans += temp_ans
        ...
        if not isinstance(input_element, (list, tuple)):
            ...
            for i in range(len(input_element)):
                new_tuple[i], temp_ans = verify_helper(
                    name+'/'+str(i), input_element[i], reference_dict['values'][i])
                ans += temp_ans
        ...
        if ok:
            if 'values' in reference_dict:
                try:
                    condition_parser().parse("{} {}".format(
                        str(input_element), str(reference_dict['values'])))
        ...
        if not isinstance(input_element, str):
            ans += [{"name": name,
                     "message": "expected a string, got {}".format(type(input_element))}]
        elif 'values' in reference_dict and not input_element in reference_dict['values']:
            ans += [{"name": name, "message": "argument must be one of the specified strings: " +
                     ", ".join(reference_dict['values'])}]
        ...
        for k in reference_dict['values']:
            if k in input_element:
                ...
                if count > 1:
                    ans += [{"name": name+"/"+k,
                             "message": "More than one argument specified for 'oneof arg: " + name}]
        if count == 0:
            if 'default' in reference_dict:
                input_element = reference_dict['default']
            else:
                ans += [{"name": name,
                         "message": "no argument provided for 'oneof' arg"}]
        ...
        ans += [{"name": name,
                 "message": "invalid type: {}".format(reference_dict['type'])}]
