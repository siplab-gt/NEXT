...
def getQuery(getQuery_args_dict, assert_200=True, verbose=False):
    ...
    response, dt = timeit(requests.post)(url, json.dumps(
        getQuery_args_dict), headers={'content-type': 'application/json'})
    if verbose:
...
def processAnswer(processAnswer_args_dict, assert_200=True, verbose=False):
    ...
    response, dt = timeit(requests.post)(url, json.dumps(
        processAnswer_args_dict), headers={'content-type': 'application/json'})
    if verbose:
        print("POST processAnswer response",
...
def format_times(getQuery_times, processAnswer_times, total_pulls,
    ...
    getQuery_times.sort()
    return_str = '%s \n\t getQuery\t : %f (5),        %f (50),        %f (95)\n\t processAnswer\t : %f (5),        %f (50),        %f (95)\n' % (participant_uid, getQuery_times[int(
        .05*total_pulls)], getQuery_times[int(.50*total_pulls)], getQuery_times[int(.95*total_pulls)], processAnswer_times[int(.05*total_pulls)], processAnswer_times[int(.50*total_pulls)], processAnswer_times[int(.95*total_pulls)])
    return return_str
