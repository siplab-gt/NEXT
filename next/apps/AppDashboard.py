...
class AppDashboard(object):
    ...
    def api_activity_histogram(self, app, butler):
        ...
        fig, ax = plt.subplots(subplot_kw=dict(
            facecolor='#FFFFFF'), figsize=(8, 1.5))
        ax.hist(numerical_timestamps, min(int(1+4*numpy.sqrt(len(numerical_timestamps))),
                300), alpha=0.5, color='black', edgecolor="black")
    ...
    def compute_duration_multiline_plot(self, app, butler, task):
        ...
        for algorithm in alg_list:
            ...
            list_of_log_dict = sorted(
                list_of_log_dict, key=lambda item: utils.str2datetime(item['timestamp']))
            ...
            for item in list_of_log_dict:
                k += 1
                x.append(k)
                y.append(item.get('app_duration', 0.) +
            ...
            incr_inds = [int(r*num_items/multiplier)
    ...
    def compute_duration_detailed_stacked_area_plot(self, app, butler, task, alg_label, detailedDB=False):
        ...
        for idx in final_inds:
            ...
            compute.append(_alg_duration -
                           _alg_duration_dbSet - _alg_duration_dbGet)
        ...
        stack_coll = ax.stackplot(
            x, compute, dbGet, dbSet, admin, enqueued, alpha=.5)
        ax.set_xlabel('API Call')
        ax.set_ylabel('Duration (s)')
        if min_x != max_x:
        ...
        proxy_rects = [plt.Rectangle((0, 0), 1, 1, alpha=.5, fc=pc.get_facecolor()[
                                     0]) for pc in stack_coll]
        legend = ax.legend(proxy_rects, [
                           'compute', 'dbGet', 'dbSet', 'admin', 'enqueued'], loc=2, ncol=3, mode="expand")
        for label in legend.get_texts():
            label.set_fontsize('small')
        plot_dict = mpld3.fig_to_dict(fig)
        plt.close()
    ...
    def response_time_histogram(self, app, butler, alg_label):
        ...
        Expected output (in dict):
          (dict) MPLD3 plot dictionary
        """
        list_of_query_dict = self.db.get_docs_with_filter(
            app.app_id+':queries', {'exp_uid': app.exp_uid, 'alg_label': alg_label})
        t = []
        ...
        if bins > 0:
            ax.hist(t, bins=bins, range=(0, 30), alpha=0.5,
    ...
    def network_delay_histogram(self, app, butler, alg_label):
        ...
        fig, ax = plt.subplots(subplot_kw=dict(facecolor='#FFFFFF'))
        ax.hist(t, MAX_SAMPLES_PER_PLOT, range=(0, 5),
                alpha=0.5, color='black', edgecolor="black")
        ax.set_xlim(0, 5)
        ax.set_axis_off()
        ax.set_xlabel('Durations (s)')
        ax.set_ylabel('Count')
