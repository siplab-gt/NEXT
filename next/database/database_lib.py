...
def make_mongodump(name, exp_uid_list=[]):
    ...
    if len(exp_uid_list) == 0:
        ...
        query_str = '\'{ $or: [ {"exp_uid":{$in:%s}}, {"object_id":{$in:%s}} ] }\'' % (
            exp_uid_list_str, exp_uid_list_str)
...
def restore_mongodump(src_filename):
    tmp_dir = tempfile.mkdtemp()
    subprocess.call(('tar -xvf {src_filename} -C {dst_path}').format(
        src_filename=src_filename, dst_path=tmp_dir), shell=True)
